...
public class PacketHandler {
  ...
  public void handlePacket(Packet packet, Session session, ByteArrayOutputStream responseStream)
    ...
    if (packet != null) {
      ...
      switch (packet.getPacketType()) {
        ...
        case OSU_LOBBY_PART ->
            handleLobbyPart((LobbyPartPacket) packet, session, responseStream);
  ...
  private void handleMessage(
      MessagePacket packet, Session session, ByteArrayOutputStream responseStream) throws IOException {
      ...
      if (channel == null) {
          logger.warn("User {} attempted to send a message to non-existent channel {}.", session.getUser().getUsername(), channelName);
          return;
      }
      ...
      if (!packet.getContent().startsWith("!help")) {
          targetSessions = channelMembersRedisService.getMembers(channel.getId());
      }
  ...
  private void handleChannelJoin(
      ChannelJoinPacket packet, Session session, ByteArrayOutputStream responseStream) throws IOException {
      Channel channel = channelService.findByName(packet.getName());

      if (channel == null) return;

      Set<UUID> currentMembers = channelMembersRedisService.getMembers(channel.getId());

      if (currentMembers.contains(session.getId())) {
          logger.warn("User {} attempted to join a channel they are already in.", session.getUser().getUsername());
          return;
      }

      channelMembersRedisService.addMemberToChannel(channel, session);
      ...
      for (Session otherOsuSession : sessionService.getAllSessions()) {
          ...
          ByteArrayOutputStream stream = new ByteArrayOutputStream();
          packetWriter.writePacket(
                  stream, new ChannelAvailablePacket(channel.getName(), channel.getTopic(), currentMembers.size() + 1));
          packetBundleService.enqueue(otherOsuSession.getId(), new PacketBundle(stream.toByteArray()));
      }

      logger.info("User {} has joined channel {}.", session.getUser().getUsername(), channel.getName());
  }

  private void handleChannelLeave(
      ChannelLeavePacket packet, Session session, ByteArrayOutputStream responseStream) throws IOException {
      Channel channel = channelService.findByName(packet.getName());

      // #lobby has its own handler
      if (channel == null || (channel.getName().equals("#lobby") && session.isReceiveMatchUpdates())) {
          return;
      }
      ...
      if (!currentMembers.contains(session.getId())) {
          logger.warn("User {} attempted to leave a channel they are not in.", session.getUser().getUsername());
          return;
      }

      channelMembersRedisService.removeMemberFromChannel(channel, session);
      ...
      for (Session otherOsuSession : sessionService.getAllSessions()) {
          ...
          packetWriter.writePacket(
                  stream, new ChannelAvailablePacket(channel.getName(), channel.getTopic(), newMemberCount));
          packetBundleService.enqueue(otherOsuSession.getId(), new PacketBundle(stream.toByteArray()));
      }
  ...
  private void handleLobbyPart(
      LobbyPartPacket packet, Session session, ByteArrayOutputStream responseStream) throws IOException {
      Channel channel = channelService.findByName(packet.getName());
      ...
      for (Session otherOsuSession : sessionService.getAllSessions()) {
          ...
          ByteArrayOutputStream stream = new ByteArrayOutputStream();
          packetWriter.writePacket(
                  stream, new ChannelAvailablePacket(channel.getName(), channel.getTopic(), currentMembers.size()));
          packetBundleService.enqueue(otherOsuSession.getId(), new PacketBundle(stream.toByteArray()));
      }
